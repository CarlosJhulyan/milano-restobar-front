...
export function LoginAdmin() {
  ...
  const { setCurrentAdmin, saveAuthAdmin } = useAuth()
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, {setStatus, setSubmitting}) => {
      setLoading(true)
      try {
        const {data: auth} = await loginAdmin(values.user, values.password);
        if (auth.success) {
          saveAuthAdmin(auth);
          setCurrentAdmin(auth.data)
  ...
  return (
    ...
    >
      <div className='text-center mb-10'>
        <h1 className='text-dark mb-3'>Administrador</h1>
      </div>
